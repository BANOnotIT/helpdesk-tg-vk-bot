...
def process_nmessage(message: NMessage):
    ...
    from_vk = message.platform is Platform.vk
    ...
    # Основные состояния бота
        ...
        if message.kind is MsgType.command:
            if message.text == '/bind':
                link = tg_link if from_vk else vk_link
                ...
                n_user = User.get_or_none(
                    user_integrating_tg if from_vk else user_integrating_vk,
                    state_param=phrase
                )

                # Скорее всего такого пользователя нет
                if n_user is None:
